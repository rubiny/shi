  ...
  if (status === 'verified') {
    return (
      <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-6">
        <div className="bg-zinc-950 border border-emerald-500/40 rounded-3xl max-w-md w-full p-10 text-center">
          <div className="text-8xl mb-6">✅</div>
          <div className="text-3xl font-bold text-emerald-400 mb-3">KYC Verified!</div>
          <div className="text-zinc-400 mb-8">Your identity has been verified. You can now make unlimited withdrawals.</div>
          <button onClick={onClose} className="w-full py-4 bg-emerald-600 rounded-2xl font-bold active:scale-[0.985]">
            CONTINUE
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[200] bg-black/95 p-6 overflow-y-auto" onClick={onClose}>
      <div className="max-w-lg mx-auto py-8" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-center gap-4 mb-8">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 1 ? 'bg-emerald-600' : 'bg-white/10'}`}>1</div>
          <div className="w-16 h-0.5 bg-white/20" />
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 2 ? 'bg-emerald-600' : 'bg-white/10'}`}>2</div>
          <div className="w-16 h-0.5 bg-white/20" />
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 3 ? 'bg-emerald-600' : 'bg-white/10'}`}>3</div>
        </div>
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-3xl p-8 space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full bg-black/60 border border-white/10 rounded-xl px-5 py-3 outline-none focus:border-emerald-500"
                placeholder="John"
              />
            </div>
            <div>
            ...
            <button
              ...
              className="w-full py-4 bg-emerald-600 rounded-2xl font-bold active:scale-[0.985] disabled:bg-zinc-800 mt-4"
            >
              CONTINUE
            </button>
          </div>
        ...
        {step === 2 && (
          <div className="bg-zinc-950 border border-white/10 rounded-3xl p-8 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {(['passport', 'id_card', 'drivers_license'] as const).map(type => (
                  <button
                    ...
                    className={`py-3 rounded-xl text-sm font-medium border transition-all ${formData.idType === type ? 'border-emerald-500 bg-emerald-500/10' : 'border-white/10'}`}
                  >
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-white/20 rounded-xl p-8 text-center hover:border-emerald-500/50 transition-all cursor-pointer">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 bg-emerald-600 rounded-2xl font-bold active:scale-[0.985] disabled:bg-zinc-800"
              >
                CONTINUE
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="bg-zinc-950 border border-white/10 rounded-3xl p-8 space-y-5">
            ...
            <div className="border-2 border-dashed border-white/20 rounded-xl p-12 text-center hover:border-emerald-500/50 transition-all cursor-pointer">
